...
import {CreateTaskFormData, User, Client, Task} from "@/hooks/type";
import { Link } from '@inertiajs/react';

const TaskCreate = ({clients}: {clients: Client[]}) => {
    const { data, setData, post, processing, errors } = useForm<CreateTaskFormData>({
        title: '',
        description: '',
        price: '',
        status: 'pending',
            ...
            onError: (errors: any) => {
                {Object.entries(errors).map(([field, error]) => (
                    toast({
                        variant: "destructive",
                        title: "Error",
                        description: String(error) || "Failed to add task. Please try again.",
                    })
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                value={data.status}
                                name={'status'}
                                onChange={e => setData('status', e.target.value as CreateTaskFormData['status'])}
                                className="mt-1 block w-full rounded-md bg-zinc-800 border-gray-600"
                            >
                        ...
                        <div>
                            ...
                            <select
                                ...
                                value={data.priority}
                                name={'priority'}
                                onChange={e => setData('priority', e.target.value as CreateTaskFormData['priority'])}
                                className="mt-1 block w-full rounded-md bg-zinc-800 border-gray-600"
                            >
